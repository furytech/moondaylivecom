  ...
  return (
    <PageLayout>
      ...
      <div className="w-full max-w-6xl mx-auto">
        ...
        {/* Sub-nav: browse other sections */}
        <nav aria-label="Journal sections" className="mb-8 flex flex-wrap items-center justify-center gap-1.5">
          <span className="text-[10px] uppercase tracking-[0.25em] text-cream-muted/70 mr-1">Browse</span>
          {FILTERS.map((f) => {
            ...
            return (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-1 rounded-full text-xs tracking-wide border transition-all ${
                  active
                    ? "border-primary/70 bg-primary/15 text-primary"
                    : "border-border/40 bg-background/40 text-cream-muted/80 hover:border-primary/40 hover:text-foreground"
                }`}
              >
                {f}
              </button>
